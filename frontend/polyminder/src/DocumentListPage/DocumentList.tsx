...
const DocumentList: React.FC = () => {
  ...
  const [useJapaneseModel, setUseJapaneseModel] = useState(false);

  const handleCheckboxChange = () => {
    setUseJapaneseModel((prev) => !prev);
  };
  ...
  // -------------------- Upload handler --------------------
  const handleUploadNewPDF = async (input: File | File[] | FileList) => {
    // Normalise everything into an array<File>
    const files: File[] = Array.isArray(input)
      ? input
      : input instanceof File
      ? [input]
      : Array.from(input);        // <- handles FileList
      ...
      >
        ...
        <DialogContent>
          ...
          {/* <FormControlLabel
            control={
              <Checkbox
                checked={useJapaneseModel}
                onChange={handleCheckboxChange}
              />
            }
            label="Use Japanese Model"
          /> */}
        </DialogContent>
      </Dialog>
    </LoadingOverlay>
