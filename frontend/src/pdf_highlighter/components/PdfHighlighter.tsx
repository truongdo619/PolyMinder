...
import TableCommentForm from "./TableCommentForm";
import LLMCommentForm from "./LLMCommentForm"
...
export const PdfHighlighter = ({
  ...
  const [llmDialogData, setLlmDialogData] = useState<{
    highlight: Highlight;
    brat_item: any;
  } | null>(null);

  const handleCloseLlmDialog = () => {
    setLlmDialogData(null);
  ...
  const custom_scrollToHighlight = (highlight: Highlight, editHighlight: (idToUpdate: string, edit: Partial<CommentedHighlight>)=> void, selectedMode: String) => {
    ...
    // ⬇️ Route to the proper dialog
    if (selectedMode === "Tables") {
      setTableDialogData({ highlight, brat_item });
    } else if (selectedMode === "LLM") {
      setLlmDialogData({ highlight, brat_item });
    } else {
  ...
  return (
    ...
    {/* ───────────────────────────────────────────────
        NEW — Table dialog (shown only in Tables mode)
      ─────────────────────────────────────────────── */}
    <Dialog
    ...
    >
      {tableDialogData && (
          <DialogContent>
            <TableCommentForm
              highlight={tableDialogData.highlight}
              brat_item={tableDialogData.brat_item}
              setTableDialogData={setTableDialogData}
              toggleEditInProgress={toggleEditInProgress}
              onSubmit={() => {
