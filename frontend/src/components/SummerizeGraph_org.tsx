...
import { 
  ...
  HelpOutline as HelpIcon // Imported Help Icon
...
export default function SummarizeGraph({ 
  ...
  // Popover state for Top-K Info
  const [anchorEl, setAnchorEl] = useState<HTMLElement | null>(null);

  // Popover state for Graph Explanation
  ...
  const handleGraphHelpOpen = (event: React.MouseEvent<HTMLElement>) => {
    setGraphHelpAnchorEl(event.currentTarget);
  };

  const handleGraphHelpClose = () => {
    setGraphHelpAnchorEl(null);
  };
  ...
  const handleSliderCommit = (event: Event | React.SyntheticEvent, newValue: number | number[]) => {
    onTopKChange(newValue as number);
  };
