...
const CommentForm = ({ onSubmit, highlight, brat_item, toggleEditInProgress, pdfHighlighterUtils }: CommentFormProps) => {
  ...
  function extractNumberFromParaId(para_id: string): number | null {
    const match = para_id.match(/\d+/);  // Regular expression to find digits
    return match ? parseInt(match[0], 10) : null; // Convert to int, or return null if no number found
}
  ...
  const handleDownload = async () => {
    ...
    if (!showAllEntities) {
      try {
        ...
        // Parse int from the highlight id
